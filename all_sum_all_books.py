...
price = soup.findAll('p', class_='count price')
count = soup.findAll('p', class_='count stock')

total = sum(float(price[number].text.split()[1].replace('$', '')) *
            float(count[number].text.split()[3])
            for number in range(len(price)))

print(f"Общая стоимость в случае продажи всех товаров: ${total}")


""" На случай не работающего сайта """
html = '''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
